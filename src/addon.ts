...
function getClientIpFromReq(req: any): string | null {
    try {
        ...
        if (xffRaw) {
            ...
            if (chosen) { if (VAVOO_CLEAN) vdbg('IP pick via XFF', { chain: parts, chosen }); return chosen; }
        }
...
// Combined resolve with plugin-aligned auth headers, IP rewriting, and TS fallback (identical to tvvoo).
...
const baseManifest: Manifest = {
    ...
    config: [
        ...
        { key: "animeunityDirectFhd", title: "AnimeUnity Synthetic FHD (solo installazione locale)", type: "checkbox" },
            ...
            try {
                ...
                if (type === "tv") {
                    ...
                    if (!dynamicHandled && (channel as any).name) {
                        ...
                        const seenVavooUrls = new Set<string>(); // Fix: deduplication Set
                            ...
                            for (const [key, value] of vavooCache.links.entries()) {
                                if (variantRegex.test(key)) {
                                    ...
                                    for (const url of links) {
                                        if (!seenVavooUrls.has(url)) { // Fix: check duplicate
                                            seenVavooUrls.add(url);
                    ...
                    if (vavooCleanPromises.length) {
                        ...
                        let inserted = 0;
                        vdbg('Clean prepend BEFORE loop', { totalVariants: vavooCleanPrepend.length, entries: vavooCleanPrepend.map((e, i) => e ? `[${i}] ${e.title}` : `[${i}] empty`) });
                ...
                if ((id.startsWith('kitsu:') || id.startsWith('mal:') || id.startsWith('tt') || id.startsWith('tmdb:')) && (trailerEnabled || animeUnityEnabled || animeSaturnEnabled || animeWorldEnabled || guardaSerieEnabled || guardoserieEnabled || guardaflixEnabled || guardaHdEnabled || eurostreamingEnabled || loonexEnabled || toonitaliaEnabled || cb01Enabled || vixsrcEnabled)) {
                    ...
                    // === GUARDOSERIE PROVIDER (Series Only) ===
                    if (guardoserieEnabled && (type as string) === 'series') {
...
app.use((req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        if (observedIp && VAVOO_CLEAN) vdbg('Observed client IP', { observedIp, reqIp: (req as any).ip, reqIps: (req as any).ips });
    } catch { }
